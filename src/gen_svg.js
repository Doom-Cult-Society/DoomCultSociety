...
function genPhrases(phraseList, initTable) {
  let codeA = '';
  let codeB = '';
  if (initTable) {
    codeA += 'mstore(table1, 0)\n';
    codeB += 'mstore(table2, 0)\n';
  }
  let ptr = 32;
  for (let i = 0; i < phraseList.length; ++i) {
    codeA += `mstore(add(table1, ${ptr}), '${phraseList[i]}')\n`;
    codeB += `mstore(add(table2, ${ptr}), ${phraseList[i].length})\n`;
    ptr += 32;
  }
  console.log(codeA + '\n' + codeB);
  console.log('len = ', phraseList.length);
}

function optimizeSvg() {
  const result = optimize(base_svg, {
...
genPhrases(phraseList1, true);
genPhrases(phraseList2, false);
genPhrases(phraseList3, false);
genPhrases(phraseList4, false);
